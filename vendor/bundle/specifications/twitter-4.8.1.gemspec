# -*- encoding: utf-8 -*-
# stub: twitter 4.8.1 ruby lib

Gem::Specification.new do |s|
  s.name = "twitter".freeze
  s.version = "4.8.1"

  s.required_rubygems_version = Gem::Requirement.new(">= 1.3.5".freeze) if s.respond_to? :required_rubygems_version=
  s.require_paths = ["lib".freeze]
  s.authors = ["John Nunemaker".freeze, "Wynn Netherland".freeze, "Erik Michaels-Ober".freeze, "Steve Richert".freeze, "Steve Agalloco".freeze]
  s.cert_chain = ["-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n".freeze]
  s.date = "2013-06-14"
  s.description = "A Ruby interface to the Twitter API.".freeze
  s.email = ["nunemaker@gmail.com".freeze, "wynn.netherland@gmail.com".freeze, "sferik@gmail.com".freeze, "steve.richert@gmail.com".freeze, "steve.agalloco@gmail.com".freeze]
  s.homepage = "http://sferik.github.com/twitter/".freeze
  s.licenses = ["MIT".freeze]
  s.rubygems_version = "2.6.11".freeze
  s.summary = "A Ruby interface to the Twitter API.".freeze

  s.installed_by_version = "2.6.11" if s.respond_to? :installed_by_version

  if s.respond_to? :specification_version then
    s.specification_version = 3

    if Gem::Version.new(Gem::VERSION) >= Gem::Version.new('1.2.0') then
      s.add_runtime_dependency(%q<faraday>.freeze, ["< 0.10", "~> 0.8"])
      s.add_runtime_dependency(%q<multi_json>.freeze, ["~> 1.0"])
      s.add_runtime_dependency(%q<simple_oauth>.freeze, ["~> 0.2"])
      s.add_development_dependency(%q<bundler>.freeze, ["~> 1.0"])
    else
      s.add_dependency(%q<faraday>.freeze, ["< 0.10", "~> 0.8"])
      s.add_dependency(%q<multi_json>.freeze, ["~> 1.0"])
      s.add_dependency(%q<simple_oauth>.freeze, ["~> 0.2"])
      s.add_dependency(%q<bundler>.freeze, ["~> 1.0"])
    end
  else
    s.add_dependency(%q<faraday>.freeze, ["< 0.10", "~> 0.8"])
    s.add_dependency(%q<multi_json>.freeze, ["~> 1.0"])
    s.add_dependency(%q<simple_oauth>.freeze, ["~> 0.2"])
    s.add_dependency(%q<bundler>.freeze, ["~> 1.0"])
  end
end
